...
- The administrator entry uses the KasmVNC core page directly without its audio/file wrapper. The management-page button transfers the current administrator login to the loopback-only entry with a random, single-use handoff that expires after 30 seconds; the long-lived session token is never placed in the URL and no second login is required. While its Kasm connection is open, desktop IPC keeps the uniquely tagged administrator X11 window in front; ordinary clipboard transactions never activate a managed workspace window or choose an administrator page by CDP Target order. A download completed in that remote administrator window is sent through the authenticated gateway entry directly to the local browser, without an administrator file viewer. Use it for ChatGPT login/sign-out, MFA, global Profile management and Chrome extension installation; it is not the normal multi-workspace transport.
...
- An ordinary workspace submits the final text produced by the access device's own browser and operating-system input method—English, Chinese, Japanese, Korean, emoji, paste or other Unicode—to its Target. Candidates and unfinished composition remain local. A remote plain-text selection is mirrored only into that viewer's hidden local input, so native `Cmd/Ctrl+C` and `Cmd/Ctrl+X` copy or cut into the access device's clipboard. ChatGPT's message and Writing Block copy buttons stay visible and transfer the resulting remote system-clipboard text to the viewer that clicked. The administrator desktop enables KasmVNC's native IME Input Mode; a local paste first sends the current browser `paste` event's plain text through Clipboard Up and then sends one remote `Ctrl+V`, so an older remote clipboard value cannot be pasted first.
...
- ChatGPT message and Writing Block copy buttons that copy the remote system clipboard into the triggering access window;
...
- rich-text, image and file clipboard formats are not mirrored; selections and ChatGPT copy buttons transfer plain text only;
