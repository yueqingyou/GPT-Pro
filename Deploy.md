...
4. 核对“普通工作区敏感操作黑名单”。默认把账号菜单、设置、退出登录、订阅、安全和破坏性全局操作保留给管理员；可按 ChatGPT 当前界面补充文字和 URL 通配规则，但已授权项目的会话操作菜单不受这些文字规则影响。
...
管理员浏览器保留 Chromium 的标签栏、地址栏和菜单。入口直接使用 KasmVNC 核心页面，不启动其音频/文件包装层；管理页点击后生成随机交接凭据，三十秒内只允许消费一次，并在本机入口复用当前管理员会话。URL 不包含长期会话令牌，不扩大 Cookie 域，也不通过 Caddy 暴露管理员监听。Kasm WebSocket 存续时，网关通过桌面 IPC 只聚焦唯一标记为 `administrator` 的 X11 顶层窗口；普通剪贴板事务不会激活工作区窗口，也不会按 CDP Target 顺序猜测管理员页面。管理员可以在这里登录或退出 ChatGPT，打开 `chrome://extensions/`，以及从 Chrome Web Store 安装扩展。远端管理员窗口完成下载后，CDP 终态经已认证实时通道送回这个本机入口，再由同源附件响应直接交给当前本机浏览器；不轮询传输目录，也不增加管理员文件查看器或 KasmVNC 文件包装层。该入口不加载普通工作区黑名单，并由管理员网关会话和独立监听共同保护。

普通工作区没有浏览器地址栏、永久顶栏或底部提示，只显示项目画布、默认下移且可直接拖动菜单图标整体移动的项目首页、刷新与菜单工具组，以及随工具组移动且默认折叠的紧凑控制面板；项目首页和刷新使用无文字的常见图标常驻显示，刷新紧邻项目首页右侧，低留白的折叠菜单只保留上传文件、查看文件、进入全屏和退出登录。控制面板标题右侧严格只显示工作区连接健康状态；上传、文件选择、剪贴板、权限和策略结果统一使用菜单外单条五秒页面反馈，后来的反馈替换前一条，不增加队列、历史或通知中心。不保留独立拖动把手。当前访问浏览器按工作区保存拖动位置，下次打开时恢复并限制在当前可见视口内。页面标题使用工作区名称，管理员浏览器标题使用 `GPT Pro`。默认黑名单同时做三件事：

1. 按非编辑控件的可见文字以及控件的可访问名称、标题、链接、测试标识和名称隐藏命中控件，并对新增 DOM 做增量复核；`input`、`textarea`、`[role="textbox"]` 或 `contenteditable` 中的用户正文及包含编辑区域的祖先容器汇总文字不参与匹配；
2. 在发送鼠标按下或键盘激活前再次检查命中控件；
3. 通过 Chromium `Network.setBlockedURLs` 阻断匹配的退出登录、停用账号等 URL。

任一受管 Target 若无法完整安装或更新黑名单，不会继续以“已就绪”状态服务；网关会关闭并按持久配置重建它。网关重启后重新认领现有 Target 时会停用页面中既有的守卫并安装当前实现，不能只替换规则数据而保留旧检查函数。
...
当前会话操作菜单使用 ChatGPT 实测的会话菜单结构作为一个整体授权面：项目专注层先拒绝分享、移动或移出项目，项目导航层再拒绝跨项目链接；通过这两层后，菜单内剩余可见操作均允许发送，包括重命名、置顶或取消置顶、归档或取消归档、删除及其会话确认。该授权仍绑定路径会话、用户工作区授权、当前 viewer 所有权与所属 CDP session，不允许客户端选择其它工作区 Target。若管理员把多个用户分配给同一工作区，这些用户本来就共享该项目及其会话操作权限，不提供同一项目内按会话创建者隔离。
...
- `Cmd/Ctrl+V` 由本机浏览器正常粘贴到隐藏文本框；网关只在当前排队事务中为所属 CDP session 模拟文档焦点，把文本写入远端 Chromium 剪贴板，再执行一次原生 Paste 编辑事务，随后立即关闭模拟且不激活工作区 X11 窗口。不程序化读取本机剪贴板，也不把大段多行文本交给 `Input.insertText`。本机粘贴与 ChatGPT 原生复制共用一个跨工作区队列，多个窗口同时操作时不会串线。
...
管理员先在完整管理员浏览器中允许 `chatgpt.com` 发送通知；这是共享 Chromium Profile 的全局权限，普通用户页面不会修改。受管 ChatGPT Page Target 实际调用原生 `Notification` 构造器后，网关只把标题和正文实时发给该 Target 所属工作区的当前连接页面，不判断普通对话、Deep Research 或其它任务类型，也不扫描完成状态。普通页面的系统通知调用点严格只保留该原生通知与下载完成、下载失败三类事件。

普通页面收到上述三类消息后，若配置的 HTTPS origin 已获普通用户浏览器的网页通知权限，则始终发送系统通知，不按页面焦点、可见性或最近输入抑制；创建成功后不重复显示页面提醒。权限不足、来源不安全、API 不支持或创建失败时，才在菜单外共用页面反馈中显示五秒，并在此期间标记标签页标题。上传、文件选择、剪贴板、权限和策略结果只使用同一页面反馈，不发送系统通知，也不覆盖菜单中的连接状态。用户首次点击普通页面时立即打开浏览器原生授权弹窗；页面加载时不申请，因为浏览器要求权限请求由用户手势触发。提醒不操作远端 ChatGPT、不改变 URL，也不发送输入。
...
Chromium 下载统一写入 `data-transfer/downloads/`。普通用户明确点击 ChatGPT 的 `Download` 或 `Download file` 后，网关会把网页的同源内容导航转换为带原文件名的 Chromium 下载；当前对话保持不变，普通页面的其它导航限制也不会放宽。网关优先让 CDP 按下载 GUID 命名；Chromium 的持久默认下载目录也固定到同一位置，因此其它 DevTools 会话重置该行为时，网关仍可依据完成事件报告的受限目录路径接管并改名，且不会读取 Profile 内的 `Downloads`。下载完成后，普通用户浏览器发送包含文件名和已保存到私人文件区的系统通知，未授权或发送失败时改为菜单外五秒页面反馈；下载失败时按相同方式显示文件名和网关返回的失败原因。两种页面反馈都不得覆盖工作区连接状态。由普通工作区触发的下载只出现在该工作区的“查看文件”面板，点击“保存到本机”后由网关鉴权并以附件响应传给当前设备。由管理员浏览器触发且无法归属工作区的下载在完成后通过管理员实时通道直接触发当前本机浏览器的附件下载；管理页仍可管理全部传输记录。
...
8. 普通工作区能从项目首页进入对话并通过常驻首页图标返回首页，右侧常驻刷新图标能刷新当前页面，折叠菜单不再重复显示刷新；菜单标题右侧只显示连接状态，上传、文件选择、剪贴板、权限或策略即时反馈在菜单外显示五秒且不能覆盖该状态；管理员 Kasm 连接活跃时，普通用户粘贴、消息复制和返回项目首页都不得把工作区窗口显示到管理员桌面最前、触发越权提示或改变管理员顶层窗口；普通编辑器粘贴包含“设置”等黑名单文字的正文后仍保持可见且发送不触发策略提示，网关重启并认领既有 Target 后也不得继续运行旧守卫，真实账号级敏感控件仍被拒绝；已授权项目内的重命名、置顶或取消置顶、归档或取消归档、删除及其会话确认可以执行，项目、对话和单条消息的分享入口仍不可见且输入前被拒绝，未获授权的工作区路径或跨项目链接不能发送输入；ChatGPT 消息复制按钮可把远端系统剪贴板定向复制到触发窗口；`@` / `+` 菜单只显示管理员按名称允许的功能，命中账号级敏感按钮或 URL 时收到提示，管理员浏览器仍可完整操作；
9. 本机上传只进入当前用户的私人目录；普通工作区手动打开文件选择器后，XDG Portal 请求必须携带该窗口的 X11 工作区标记并只显示近期输入所属用户的私人文件，确认后 Chromium 页面收到标准 `FileList`；源码与运行日志中不存在 CDP 文件选择拦截或 DOM 文件输入写入，管理员窗口仍出现 GTK 原生文件选择器；普通远端下载成功或失败时已授权浏览器发送系统通知且不重复显示页面反馈，未授权或发送失败时在菜单外显示五秒页面反馈，文件只能由所属工作区或管理员取回；管理员浏览器触发的真实下载完成后，当前本机浏览器直接收到同名附件。
10. 管理员浏览器已允许 `chatgpt.com` 通知，真实 ChatGPT 完成通知的标题正文只转发到所属工作区的普通页面；该页面已授权时始终发送系统通知且不重复显示页面反馈，未授权或发送失败时显示菜单外五秒页面反馈，其它工作区不收到。
...
- 所有工作区 Target 与管理员浏览器共用同一个 Chromium BrowserContext、Profile、ChatGPT 身份和站点存储。网关按所属 CDP session 定向输入，但不隔离 ChatGPT 自身保存或恢复的未发送草稿；管理员页面刷新后出现同一草稿不代表 CDP 输入被发送到管理员 Target。若要隔离这类应用级状态，必须改为独立 BrowserContext 或 Profile，并重新定义登录态共享边界，不能作为当前单 Profile 架构的小修实现。
...
- 编辑器功能白名单依赖 ChatGPT 当前的完整功能名和菜单 DOM，黑名单依赖管理员维护的非编辑控件文字、控件元数据和 URL 通配规则；项目专注层还依赖当前项目 URL 与分享控件属性。控件隐藏、网关输入检查、网络 URL 阻断和项目导航规则均需在 ChatGPT 改版后复核，不能视为对未知未来端点的永久证明或不可探查机制。
