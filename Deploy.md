...
- `Cmd/Ctrl+V` 由本机浏览器正常粘贴到隐藏文本框，再作为最终文本注入远端，不请求程序化剪贴板读取权限。每个本地页面拥有自己的组合状态，一个窗口正在选词不会改变另一个窗口。
...
- 本机粘贴、远端纯文本选区复制/剪切和 ChatGPT 消息复制按钮均可用；消息按钮经全局原子队列把远端系统剪贴板定向写入触发窗口，富文本、图片和文件剪贴板格式不做同步，文件通过工作区文件面板双向流通。
