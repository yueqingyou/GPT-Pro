...
8. 普通工作区能从项目首页进入对话并通过常驻首页图标返回首页，右侧常驻刷新图标能刷新当前页面，折叠菜单不再重复显示刷新；菜单标题右侧只显示连接状态，上传、文件选择、剪贴板、权限或策略即时反馈在菜单外显示五秒且不能覆盖该状态；已授权项目内的重命名、置顶或取消置顶、归档或取消归档、删除及其会话确认可以执行，项目、对话和单条消息的分享入口仍不可见且输入前被拒绝，未获授权的工作区路径或跨项目链接不能发送输入；ChatGPT 消息复制按钮可把远端系统剪贴板定向复制到触发窗口；`@` / `+` 菜单只显示管理员按名称允许的功能，命中账号级敏感按钮或 URL 时收到提示，管理员浏览器仍可完整操作；
