...
const ACTION_CANDIDATE_SELECTOR =
  "a,button,[role='button'],[role='menuitem'],[role='link'],[role='textbox'],[contenteditable]:not([contenteditable='false']),[tabindex],input,label,textarea";
const EDITABLE_ACTION_SELECTOR =
  "input,textarea,[role='textbox'],[contenteditable]:not([contenteditable='false'])";

function inspectSensitiveAction(element, candidateSelector, editableSelector) {
  ...
  const tagName = candidate.tagName.toLocaleLowerCase("en-US");
  const role = candidate.getAttribute("role") || "";
  const editable =
    candidate.isContentEditable ||
    tagName === "input" ||
    tagName === "textarea" ||
    role.toLocaleLowerCase("en-US") === "textbox" ||
    !!candidate.querySelector(editableSelector);
  const text = editable ? "" : String(candidate.textContent || "").trim().slice(0, 512);
  const ariaLabel = candidate.getAttribute("aria-label") || "";
  const testId = candidate.getAttribute("data-testid") || "";
  const name = candidate.getAttribute("name") || "";
  const conversationTriggerSelector =
    '[data-testid="conversation-options-button"], [aria-label^="Open conversation options for "]';
  const conversationItem = candidate.closest("li");
  const conversationDialog = candidate.closest('[role="dialog"]');
  const description = [
    text,
    ariaLabel,
  ...
  return {
    ...
    conversationAction:
      ...
      ariaLabel.startsWith("Open conversation options for ") ||
      !!conversationItem?.querySelector(conversationTriggerSelector) ||
      (role === "tabpanel" && !!candidate.querySelector(conversationTriggerSelector)) ||
      !!conversationDialog?.querySelector('[data-testid="delete-conversation-confirm-button"]') ||
      (role === "menuitem" &&
        !!candidate.closest('[role="menu"]')?.querySelector('[data-testid="delete-chat-menu-item"]')),
  };
}
...
export function sensitiveActionMatch(policy, action, allowConversationActions) {
  if (!policy?.enabled || (allowConversationActions && action?.conversationAction)) return "";
  const text = String(action?.description || "").normalize("NFKC").toLocaleLowerCase("en-US");
...
export function sensitiveActionInspectExpression(targetExpression) {
  return `(${inspectSensitiveAction.toString()})(${targetExpression}, ${JSON.stringify(ACTION_CANDIDATE_SELECTOR)}, ${JSON.stringify(EDITABLE_ACTION_SELECTOR)})`;
}

export function sensitiveGuardScript(policy, allowConversationActions) {
  const initial = {
    enabled: !!policy?.enabled,
    actionPatterns: [...(policy?.actionPatterns || [])],
    allowConversationActions: !!allowConversationActions,
  };
  return `(() => {
    const normalize = (value) => String(value || "").normalize("NFKC").toLocaleLowerCase("en-US");
    const install = () => {
      const previous = globalThis.__gpcSensitiveGuard;
      if (previous) {
        if (typeof previous.dispose === "function") previous.dispose();
        else previous.update({ enabled: false, actionPatterns: [], allowConversationActions: false });
      }
      let policy = ${JSON.stringify(initial)};
      const candidateSelector = ${JSON.stringify(ACTION_CANDIDATE_SELECTOR)};
      const editableSelector = ${JSON.stringify(EDITABLE_ACTION_SELECTOR)};
      // 已部署页面的旧观察器无法断开且会继续清理旧属性，当前守卫必须使用独立属性。
      const hiddenAttribute = "data-gpc-sensitive-control-hidden";
      const styleSelector = "style[data-gpc-sensitive-control-style]";
      const inspect = ${inspectSensitiveAction.toString()};
      let observer = null;
      let disposed = false;
      const match = (element) => {
        if (!policy.enabled) return "";
        const action = inspect(element, candidateSelector, editableSelector);
        if (policy.allowConversationActions && action.conversationAction) return "";
        const value = normalize(action.description);
        return policy.actionPatterns.find((pattern) => value.includes(normalize(pattern))) || "";
      };
      const ensureStyle = () => {
        if (!document.documentElement || document.querySelector(styleSelector)) return;
        const style = document.createElement("style");
        style.dataset.gpcSensitiveControlStyle = "true";
      ...
      const dispose = () => {
        disposed = true;
        policy = { enabled: false, actionPatterns: [], allowConversationActions: false };
        observer?.disconnect();
        observer = null;
      ...
      observe();
      globalThis.__gpcSensitiveGuard = { update, dispose };
    };
    install();
  })();`;
}
