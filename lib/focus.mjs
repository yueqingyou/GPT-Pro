...
export function composerMenuFromElement(element) {
  const menu = element.closest(".popover");
  if (!menu || !menu.querySelector(".__menu-item")) return null;
  const composerTrigger = '[data-testid="composer-plus-btn"][aria-expanded="true"]';
  let current = menu;
  while (current) {
    if (current.parentElement.querySelector(composerTrigger)) return menu;
    if (!current.id) return null;
    const trigger = document.querySelector('[aria-controls="' + CSS.escape(current.id) + '"]');
    if (!trigger) return null;
    if (trigger.matches(composerTrigger)) return menu;
    current = trigger.closest(".popover");
  }
  return null;
}

export function composerMenuInspectExpression(targetExpression) {
  return `(${composerMenuFromElement.toString()})(${targetExpression})`;
}
...
export function projectFocusScript(allowedTools = DEFAULT_COMPOSER_TOOL_ALLOWLIST) {
  ...
  return `(() => {
    ...
    const composerMenu = ${composerMenuFromElement.toString()};
    const install = () => {
      ...
      const apply = (root) => {
        for (const menu of select(root, ".popover")) {
