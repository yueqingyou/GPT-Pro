...
const DEFAULT_QUOTA_BYTES = 4 * 1024 * 1024 * 1024;
...
function bytesLimit(value, label, minimum = 1024 * 1024) {
...
export function createTransferStore({
  ...
  const perFileLimit = bytesLimit(maxFileBytes, "单文件容量上限");
  const totalLimit = bytesLimit(quotaBytes, "传输目录容量上限", perFileLimit);
  const requestedTtl = Number(uploadTtlMs);
  const ttl = Number.isSafeInteger(requestedTtl) && requestedTtl >= 60 * 1000
    ? requestedTtl
  ...
  const usedBytes = (exceptId = "") => entries.reduce(
    (sum, entry) => sum + (entry.id === exceptId ? 0 : entryBytes(entry)),
    0,
  );
  ...
  const api = {
    ...
    quotaBytes: totalLimit,
    cleanup() {
      const cutoff = now() - ttl;
      const kept = [];
      let changed = false;
      for (const entry of entries) {
        const expiredUpload = entry.kind === "upload" && Date.parse(entry.updatedAt) < cutoff;
        const missingReadyFile = entry.state === "ready" && !existsSync(entryPath(entry));
        if (expiredUpload || missingReadyFile || entry.state === "receiving") {
    ...
    async receiveUpload(stream, { userId, name, mimeType, declaredSize = 0 } = {}) {
      ...
      if (size > perFileLimit) throw fail(`单个文件不能超过 ${perFileLimit} 字节`, 413);
      if (usedBytes() + size > totalLimit) throw fail("文件传输目录已达到容量上限", 507);
      const id = randomUUID();
      ...
      const limit = new Transform({
        transform(chunk, _encoding, callback) {
          ...
          if (received > perFileLimit || usedBytes(id) + received > totalLimit) {
            callback(fail("上传文件超过容量限制", 413));
            return;
          }
          callback(null, chunk);
        },
    ...
    updateDownload({ id, state, receivedBytes = 0, totalBytes = 0, filePath = "", finalAttempt = false }) {
      ...
      entry.updatedAt = new Date(now()).toISOString();
      const projected = Math.max(entry.receivedBytes, entry.totalBytes);
      let cancel = projected > perFileLimit || usedBytes(entry.id) + projected > totalLimit;
      const reportedPath = reportedDownloadPath(filePath);
      if (cancel) {
        entry.state = "failed";
        entry.error = "下载超过文件或目录容量限制";
        removeEntryFiles(entry);
        if (reportedPath && reportedPath !== entryPath(entry)) rmSync(reportedPath, { force: true });
        persist();
        ...
        if (existsSync(path)) {
          ...
          cancel = actualSize > perFileLimit || usedBytes(entry.id) + actualSize > totalLimit;
          if (cancel) {
            entry.state = "failed";
            entry.error = "下载超过文件或目录容量限制";
            removeEntryFiles(entry);
            persist();
            return { entry: publicEntry(entry), cancel, retry: false, terminal: true };
          }
          setOwner(path);
    ...
    resolveUserUploads({ userId, ids }) {
      if (!Array.isArray(ids) || !ids.length || ids.length > 10) throw fail("请选择 1–10 个待上传文件");
      const privateUserId = String(userId || "");
      const result = [];
