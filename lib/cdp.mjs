...
const NOTIFICATION_BINDING = "__gpcNotification";
...
const DOWNLOAD_ACTION_LABELS = new Set(["download", "download file", "下载", "下载文件"]);
...
function notificationRelayScript() {
  return `(() => {
    if (globalThis.__gpcNotificationRelayInstalled || typeof globalThis.Notification !== "function") return;
    const binding = ${JSON.stringify(NOTIFICATION_BINDING)};
    const NativeNotification = globalThis.Notification;
    const NotificationProxy = new Proxy(NativeNotification, {
    ...
    Object.defineProperty(globalThis, "__gpcNotificationRelayInstalled", {
      configurable: true,
      value: true,
    });
  })()`;
...
export class WorkspaceBroker {
  constructor({
    ...
    this.activeFrameFps = boundedInteger(activeFrameFps, 1, 60, 60);
    this.jpegQuality = boundedInteger(jpegQuality, 35, 90, 72);
    this.defaultViewport = {
      width: boundedInteger(viewport.width, MIN_VIEWPORT_WIDTH, SCREENCAST_MAX_WIDTH, 1440),
      height: boundedInteger(viewport.height, MIN_VIEWPORT_HEIGHT, SCREENCAST_MAX_HEIGHT, 900),
    };
    ...
    this.stopped = false;
    this.portal?.on("open", (request) => {
      this.#handlePortalOpen(request).catch((error) => {
        this.portal.cancel(request.requestId);
        this.logger.warn?.(`处理文件 Portal 请求失败：${error.message}`);
      });
    });
    this.portal?.on("closed", (request) => this.#handlePortalClosed(request));
  }
  ...
  status() {
    return {
      ...
      streamMaxWidth: SCREENCAST_MAX_WIDTH,
      streamMaxHeight: SCREENCAST_MAX_HEIGHT,
      streamQuality: this.jpegQuality,
    };
  }

  viewerCountsByWorkspace() {
    return Object.fromEntries([...this.viewers].map(([workspaceId, viewers]) => [workspaceId, viewers.size]));
  }
  ...
  stop() {
    ...
    this.reconnectTimer = null;
    for (const runtime of this.runtimes.values()) this.#clearFileChooser(runtime);
  ...
  async #ensureConnection() {
    ...
    this.connecting = (async () => {
      ...
      try {
        ...
        await this.portal?.tagAdministrator();
        if (this.maintenanceActive) await this.#focusMaintenanceWindow();
        this.discovering = false;
        this.reconnectDelayMs = 500;
        this.connectionWarningShown = false;
  ...
  #handleEvent(connection, message) {
    ...
    if (message.method === "Browser.downloadWillBegin" && this.transferStore) {
      try {
        ...
        if (runtime) runtime.downloadRequest = null;
    ...
    if (message.method === "Browser.downloadProgress" && this.transferStore) {
      try {
        ...
        if (result.entry && result.terminal) {
          if (workspaceId) this.#broadcast(workspaceId, { type: "download", file: result.entry });
          else this.#notifyAdminDownload(result.entry);
        }
        if (result.cancel) {
    ...
    if (message.method === "Runtime.bindingCalled" && runtime && message.params?.name === NOTIFICATION_BINDING) {
      try {
        const notification = JSON.parse(message.params.payload);
        if (typeof notification.title !== "string" || typeof notification.body !== "string") return;
        this.#broadcast(workspaceId, { type: "notification", ...notification });
      } catch {
        return;
      }
      return;
    ...
    if (
      (message.method === "Page.frameScheduledNavigation" || message.method === "Page.frameRequestedNavigation") &&
      runtime &&
      message.params?.frameId === runtime.mainFrameId &&
      this.#convertDownloadNavigation(connection, runtime, message.params?.url)
    ) {
      return;
    }
  ...
  #handleMainNavigation(connection, runtime, url) {
    ...
    runtime.downloadRequest = null;
    this.#clearFileChooser(runtime);
  ...
  #clearFileChooser(runtime) {
    if (runtime.fileChooser) this.portal?.cancel(runtime.fileChooser.requestId);
    runtime.fileChooser = null;
  }

  async #handlePortalOpen(request) {
    const runtime = this.runtimes.get(request.workspaceId);
    const owner = runtime?.fileChooserOwner;
    if (runtime) runtime.fileChooserOwner = null;
    if (
      this.stopped ||
      request.directory ||
      !runtime?.ready ||
      ...
      !this.viewers.get(request.workspaceId)?.has(owner.viewer)
    ) {
      this.portal.cancel(request.requestId);
      return;
    }
    ...
    this.#sendJson(owner.viewer, {
      type: "file-chooser",
      requestId: request.requestId,
      mode: request.multiple ? "selectMultiple" : "selectSingle",
    });
  }

  #handlePortalClosed(request) {
    const runtime = this.runtimes.get(request.workspaceId);
    if (runtime?.fileChooser?.requestId !== request.requestId) return;
    const viewer = runtime.fileChooser.viewer;
    runtime.fileChooser = null;
    this.#sendJson(viewer, { type: "file-selection-cancelled", requestId: request.requestId });
  }
  ...
  async #inputActionAt(connection, runtime, { x, y, active = false }) {
    ...
    const expression = `(() => {
      ...
      const candidate = target.closest('.__menu-item,[role="menuitem"],[role="menuitemradio"],button') || target;
      const menu = ${composerMenuInspectExpression("candidate")};
      const dialog = candidate.closest('[role="dialog"]');
      const composerTool = !!menu;
      ...
      const actionLabel = String(action.ariaLabel || "").trim().toLocaleLowerCase("en-US");
      const writingBlockCopy = actionLabel === "copy" &&
        !!candidate.closest('[data-testid="writing-block-header-surface"]');
      const download = ${JSON.stringify([...DOWNLOAD_ACTION_LABELS])}.includes(actionLabel);
      const headerName = candidate.closest("header")?.querySelector("[data-file-name-full-text]")
        ?.getAttribute("data-file-name-full-text");
      const artifactRow = candidate.closest('[class*="group/artifact-row"]');
      const artifactName = [...(artifactRow?.querySelectorAll("button[aria-label]") || [])]
        .find((element) => element !== candidate && !${JSON.stringify([...DOWNLOAD_ACTION_LABELS])}.includes(
          String(element.getAttribute("aria-label") || "").trim().toLocaleLowerCase("en-US"),
      ...
      return {
        ...
        writingBlockCopy,
        download,
        downloadName: download ? String(headerName || artifactName || "download").trim().slice(0, 180) : "",
      };
    })()`;
  ...
  #queueRemoteClipboardPaste(connection, runtime, text, viewer) {
    const operation = this.clipboardQueue.then(() => this.#pasteRemoteClipboard(connection, runtime, text, viewer));
    this.clipboardQueue = operation.catch(() => {});
    return operation;
  }

  async #withRemoteClipboardContext(connection, runtime, operation) {
    await connection.call("Emulation.setFocusEmulationEnabled", { enabled: true }, runtime.sessionId);
    try {
      ...
      return await operation(executionContextId);
    } finally {
      try {
        await connection.call("Emulation.setFocusEmulationEnabled", { enabled: false }, runtime.sessionId);
      } finally {
        if (this.maintenanceActive) await this.#focusMaintenanceWindow();
      }
    }
  ...
  async #pasteRemoteClipboard(connection, runtime, text, viewer) {
    await this.#withRemoteClipboardContext(connection, runtime, async (executionContextId) => {
      const written = await connection.call(
        "Runtime.evaluate",
        {
      ...
      if (written.exceptionDetails || written.result?.value !== true) throw new Error("无法写入远端剪贴板");
      this.#assertViewerOwnership(runtime.workspaceId, viewer);
  ...
  async #copyRemoteClipboard(connection, runtime, viewer, mouseParams) {
    if (!viewer) throw new Error("画面连接不属于当前工作区");
    this.#assertViewerOwnership(runtime.workspaceId, viewer);
    const text = await this.#withRemoteClipboardContext(connection, runtime, async (executionContextId) => {
  ...
  #assertViewerOwnership(workspaceId, viewer) {
    if (viewer && !this.viewers.get(workspaceId)?.has(viewer)) {
  ...
  #blockInputAction(workspaceId, runtime, action) {
    ...
    const pattern = sensitiveActionMatch(this.store.sensitivePolicy(), action, !!runtime.projectScope);
    if (!pattern) return false;
    this.#broadcastPolicyBlock(workspaceId, pattern);
    return true;
  }

  #convertDownloadNavigation(connection, runtime, rawUrl) {
    const request = runtime.downloadRequest;
    if (!request) return false;
    runtime.downloadRequest = null;
    let url;
    try {
      url = new URL(rawUrl);
      if (url.origin !== new URL(runtime.safeUrl).origin) return false;
    } catch {
      return false;
    }
    const expression = `(() => {
      const anchor = document.createElement("a");
      anchor.href = ${JSON.stringify(url.toString())};
      anchor.download = ${JSON.stringify(request.name)};
      document.body.append(anchor);
      anchor.click();
      anchor.remove();
    })()`;
    connection
      .call("Page.stopLoading", {}, runtime.sessionId)
      .then(() => connection.call("Runtime.evaluate", { expression, userGesture: true }, runtime.sessionId))
      .catch((error) => {
        this.#broadcast(runtime.workspaceId, {
          type: "download",
          file: { name: request.name, state: "failed", error: "无法启动下载" },
        });
        this.logger.warn?.(`转换 ChatGPT 下载失败：${error.message}`);
      });
    return true;
  }
  ...
  async #applySensitivePolicyToRuntime(connection, runtime) {
    ...
    const source = sensitiveGuardScript(policy, !!runtime.projectScope);
  ...
  #selectUploadFiles(runtime, uploads, chooser) {
    if (!uploads?.length) throw new Error("没有可上传文件");
    if (!chooser?.multiple && uploads.length > 1) {
      throw new Error("当前上传入口一次只接受一个文件，请分次选择");
    }
    if (!this.portal?.select(chooser.requestId, uploads.map((upload) => upload.path))) {
      throw new Error("当前文件选择已经关闭");
    }
    runtime.fileChooser = null;
  ...
  async #prepareRuntime(connection, workspace, targetId, sessionId, { tagWindow = false } = {}) {
    await connection.call("Page.enable", {}, sessionId);
    await connection.call("Runtime.enable", {}, sessionId);
    await connection.call("Network.enable", {}, sessionId);
    await connection.call("Runtime.addBinding", { name: NOTIFICATION_BINDING }, sessionId);
    if (tagWindow) {
      if (!this.portal) throw new Error("当前部署没有启用文件 Portal");
      const temporaryTitle = `gpc-${randomUUID()}`;
      await connection.call(
        "Runtime.evaluate",
        { expression: `document.title = ${JSON.stringify(temporaryTitle)}` },
        sessionId,
      );
      await this.portal.tagWorkspace({ title: `${temporaryTitle} - Chromium`, workspaceId: workspace.id });
    }
    const runtime = {
    ...
    try {
      ...
      await connection.call(
        "Page.addScriptToEvaluateOnNewDocument",
        { source: notificationRelayScript(), runImmediately: true },
        sessionId,
      );
      await this.#applyBrowserProfileToSession(connection, sessionId);
  ...
  async #createRuntime(workspace, connection) {
    ...
    try {
      ({ sessionId } = await connection.call("Target.attachToTarget", { targetId, flatten: true }));
      const runtime = await this.#prepareRuntime(connection, workspace, targetId, sessionId, { tagWindow: true });
  ...
  async #focusMaintenanceWindow() {
    if (!this.portal) throw new Error("当前部署没有启用桌面窗口控制");
    await this.portal.focusAdministrator();
  }

  async focusMaintenance() {
    const focusDuringConnect = this.maintenanceActive && (!this.connection?.open || !!this.connecting);
    await this.#ensureConnection();
    if (!focusDuringConnect) await this.#focusMaintenanceWindow();
  }
  ...
  async addViewer(workspaceId, socket) {
    ...
    this.viewerStates.set(socket, { visible: true });
    this.#notifyState();
  ...
  removeViewer(workspaceId, socket) {
    const viewers = this.viewers.get(workspaceId);
    if (!viewers) return;
    if (!viewers.delete(socket)) return;
  ...
  async #setViewerState(workspaceId, socket, { visible = true } = {}) {
    const state = { visible: !!visible };
    this.viewerStates.set(socket, state);
    const runtime = this.runtimes.get(workspaceId);
    if (runtime) await this.#syncScreencast(runtime);
    return state;
  }
  ...
  #screencastRate(runtime) {
    ...
    return {
      fps: this.activeFrameFps,
      maxWidth: runtime.viewport.width,
      maxHeight: runtime.viewport.height,
  ...
  async #setWindowVisible(connection, runtime, visible) {
    ...
    if (visible && this.maintenanceActive && !this.discovering) await this.#focusMaintenanceWindow();
  }
  ...
  async handleCommand(workspaceId, command, actor = null, viewer = null) {
    ...
    if (
      (viewer || ["viewerState", "selection", "cut"].includes(command.type)) &&
      !this.viewers.get(workspaceId)?.has(viewer)
    ) {
      throw new Error("画面连接不属于当前工作区");
    }
    ...
    const connection = await this.#ensureConnection();
    const runtime = await this.ensureWorkspace(workspaceId);
    this.#assertViewerOwnership(workspaceId, viewer);
    ...
    if (command.type === "selectFiles") {
      ...
      if (chooser.requestId !== command.requestId) throw new Error("当前文件选择已经更换");
      ...
      this.#selectUploadFiles(runtime, uploads, chooser);
      return;
    }

    if (command.type === "cancelFileSelection") {
      if (
        runtime.fileChooser?.requestId === command.requestId &&
        runtime.fileChooser.userId === actor?.id &&
        runtime.fileChooser.viewer === viewer
      ) {
        this.#clearFileChooser(runtime);
      }
      return;
    ...
    if (command.type === "selection") {
      const text = await this.#selectionText(connection, runtime);
      this.#assertViewerOwnership(workspaceId, viewer);
      this.#sendJson(viewer, { type: "selection", text });
      return;
    }
    ...
    if (command.type === "resize") {
      const viewport = {
        width: boundedInteger(command.width, MIN_VIEWPORT_WIDTH, SCREENCAST_MAX_WIDTH, runtime.viewport.width),
        height: boundedInteger(command.height, MIN_VIEWPORT_HEIGHT, SCREENCAST_MAX_HEIGHT, runtime.viewport.height),
      };
    ...
    if (command.type === "pointer") {
      ...
      if (eventType === "mousePressed") {
        ...
        if (action.download === true) runtime.downloadRequest = { name: action.downloadName };
        if (viewer) {
    ...
    if (command.type === "text") {
      ...
      if (command.paste === true) await this.#queueRemoteClipboardPaste(connection, runtime, text, viewer);
      else await connection.call("Input.insertText", { text }, sessionId);
      return;
    }
