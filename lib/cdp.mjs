...
export class WorkspaceBroker {
  ...
  #queueRemoteClipboardPaste(connection, runtime, text) {
    const operation = this.clipboardQueue.then(() => this.#pasteRemoteClipboard(connection, runtime, text));
    this.clipboardQueue = operation.catch(() => {});
    return operation;
  }
  ...
  async #pasteRemoteClipboard(connection, runtime, text) {
  ...
  async handleCommand(workspaceId, command, actor = null, viewer = null) {
    ...
    if (command.type === "text") {
      ...
      if (command.paste === true) await this.#queueRemoteClipboardPaste(connection, runtime, text);
