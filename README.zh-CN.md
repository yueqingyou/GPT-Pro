...
- 每个普通网关用户在全部获授权工作区中只能有一个活跃观看连接。打开新窗口后立即接管，旧页面停止画面与输入且不再自动重连；不同用户仍可同时在线，也可以连接同一工作区。
...
- 每个普通用户只能保留一个活跃工作区 WebSocket；跨工作区打开新窗口会替换旧连接，旧页面停止而不自动重连，但路径独立登录仍保留，可由用户明确重新载入后接管。
