...
- 本机上传只保存到当前用户的私人目录，不会自动交给 ChatGPT。每个用户有 1 GiB 私人上传空间；文件保留到用户或管理员手动删除，不另设单文件或全服务器应用级上限。用户在 ChatGPT 页面点击上传后，Chromium 发出的 Linux XDG Desktop Portal FileChooser 请求按触发窗口路由到工作台私人文件面板；用户确认后，由 Chromium 从返回的本机文件 URI 生成标准网页 `FileList`。传输过程不使用 CDP 文件选择拦截、DOM 文件输入写入、网页选择器或坐标自动化；管理员窗口仍使用 GTK 原生文件选择器。用户明确触发的 ChatGPT 下载会转换为当前工作区的 Chromium 下载并只经过独立传输目录；下载完成或失败后，已获权限的普通用户浏览器发送系统通知，否则在右上角显示包含文件名且五秒后自动关闭的页面提醒。普通页面导航限制不因此放宽，gateway 也不挂载 Chromium Profile。
